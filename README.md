...
감시 시간은 `watch_hours`(한국 시간, 평일 07:50~19:00, 주말·공휴일 제외)입니다. PC/VM 감시와 GitHub Actions 모두 이 시간 밖에서는 아무것도 하지 않습니다. 공휴일(대체공휴일·선거일 포함)은 `holidays` 패키지가 판단하고, 거기에 아직 없는 임시공휴일은 `extra_holidays`에 날짜를 추가하세요. `watch_hours`를 지우면 24시간 매일 감시합니다. 시간 값은 반드시 따옴표로 감싸세요.
